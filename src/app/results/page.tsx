...
export default function ResultsPage() {
    ...
    const handleDownloadReport = () => {
        if (!apiData || !apiData.aiData) return;

        const { aiData } = apiData;
        const timestamp = new Date().toLocaleDateString();

        const reportContent = `GREENBUILD AI - SUSTAINABILITY ADVISOR REPORT
DATE: ${timestamp}
PROJECT: ${specs.type} - ${specs.location}
AREA: ${specs.area.toLocaleString()} sq ft | FLOORS: ${specs.floors}

=== EXECUTIVE SUMMARY ===
${aiData.impactSummary}

=== CARBON HOTSPOTS ===
${aiData.hotspots.map((h: any) => `- ${h.material}: ${h.reason}`).join('\n')}

=== OPTIMIZATION STRATEGY (XAI) ===
${aiData.optimizations.map((o: any) => `
[${o.title}]
Action: ${o.action}
Impact: -${o.carbonSavingTons} Tons CO2e
Cost Delta: $${o.costDeltaUsd}
Technical Reason: ${o.technicalExplanation}
Tradeoff: ${o.tradeoff}
`).join('\n')}

=== POLICY INSIGHT ===
${aiData.policyInsight}

Generated by GreenBuild AI
`;
        ...
        a.href = url;
        a.download = `GreenBuild_Report_${specs.type.replace(/\s+/g, '_')}.txt`;
    ...
    const handleDownloadBOM = () => {
        if (!currentResults?.materials) return;

        const headers = ["Material", "Quantity", "Unit", "Total Carbon (Tons)", "Total Cost ($)"];
        const rows = currentResults.materials.map((m: any) => [
            `"${m.material}"`,
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `BOM_${specs.type.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 pb-20 dark:bg-slate-950">
            ...
            <header className="border-b bg-background/80 backdrop-blur-xl sticky top-0 z-50">
                <div className="container mx-auto px-4 h-18 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href={user ? "/dashboard" : "/analyze"} className="p-2 hover:bg-muted rounded-xl transition-all">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {user && (
                            <Button variant="outline" size="sm" onClick={handleSaveProject} className="hidden sm:flex rounded-full border-slate-200 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200">
            ...
            <main className="container mx-auto px-4 py-8 space-y-8 max-w-7xl">
                ...
                {!apiData.aiData ? (
                    <div className="p-12 text-center bg-slate-50 dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700">
                        <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4">
                            <Sparkles className="w-8 h-8 text-slate-400" />
                        </div>
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">AI Insights Unavailable</h3>
                        <p className="text-muted-foreground max-w-md mx-auto">
                            The AI reasoning engine could not generate insights for this configuration. This might be due to a strict content filter or a temporary connection issue.
                        </p>
                    </div>
                ) : (
                ...
                <Card className="border-none shadow-2xl bg-gradient-to-r from-slate-900 to-indigo-950 text-white p-8 overflow-hidden relative">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-center relative z-10">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 shrink-0">
                            ...
                            <Button
                                size="lg"
                                className="rounded-full bg-primary hover:bg-primary/90 text-white border-none h-14 px-10 font-black shadow-xl shadow-primary/20 text-lg group"
